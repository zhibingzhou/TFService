...
//玖付初始化
func (this *JPAY) Init(pay_id string) {
	...
	if len(pay_conf["id"]) > 0 {
		...
		if len(res) > 0 {
			...
			if err != nil {
				common.LogsWithFileName(log_path, "JPAY_init_", "error->"+err.Error())
			}
		}
	}
...
func (api *JPAY) CreatePay(p *PayData) (int, string, string, string, string, map[string]string) {
	api_method := "POST"
	re_status := 100
	re_msg := "请求错误"
...
//回调验证
func (api *JPAY) CallBackPay(sign, sign_str string) int {
	result := 101
	...
	common.LogsWithFileName(log_path, "JPAY_sign_", log_str)
	return result
}

//代付
func (api *JPAY) PayFor(pay_data map[string]string) (int, string, string) {
	//定义初始值
	api_status := 200
...
func (api *JPAY) PayQuery(pay_data map[string]string) (int, string, string) {
	//定义初始值
	api_status := 200
	pay_result := "processing"
	api_msg := "success"
