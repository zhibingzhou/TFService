...
//015支付
func (this *ZOFPAY) Init(pay_id string) {
	...
	if len(pay_conf["id"]) > 0 {
		...
		if len(res) > 0 {
			...
			if err != nil {
				common.LogsWithFileName(log_path, "ZOFPAY_init_", "error->"+err.Error())
			}
		}
	}
...
func (api *ZOFPAY) CreatePay(p *PayData) (int, string, string, string, string, map[string]string) {
	api_method := "POST"
	re_status := 100
	re_msg := "请求错误"
...
//回调验证
func (api *ZOFPAY) CallBackPay(sign, sign_str string) int {
	result := 101
	...
	common.LogsWithFileName(log_path, "zof_sign_", log_str)
	return result
}

//代付
func (api *ZOFPAY) PayFor(pay_data map[string]string) (int, string, string) {
	//定义初始值
	api_status := 200
...
func (api *ZOFPAY) PayQuery(pay_data map[string]string) (int, string, string) {
	//定义初始值
	api_status := 200
	pay_result := "processing"
	api_msg := "success"
